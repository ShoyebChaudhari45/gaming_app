...
public class SessionManager {
    ...
    public static double getBalance(Context context) {
        ...
        try {
            String balanceStr = prefs.getString(KEY_BALANCE, "0");
            return Double.parseDouble(balanceStr);
        } catch (ClassCastException e) {
            // Backward compatibility: old version stored as int
            try { return prefs.getInt(KEY_BALANCE, 0); } catch (Exception ex) { return 0; }
    ...
    public static String getFormattedBalance(Context context) {
        return formatBalance(getBalance(context));
    }
